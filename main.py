    ...
    for r in range(1, 5):
        for c in range(4):
            ...
            btn = tk.Button(
                ...
                text="  ",
                image=photo,
                command=lambda btn_num=btn_num: click_btn(btn_num),
            )
            btn.config(width=100, height=100)
            buttons.append(btn)
            btn.place(x=81 * c, y=81 * r)
...
title = tk.Label(root, image="C:\\Users\\user\\Desktop\\high\\title.png")
...
rank1 = tk.Label(
    root,
    text=f"1: {rank[0][0] if len(rank) > 0 else '-'} {rank[0][1] if len(rank) > 0 else '-'}",
)
rank2 = tk.Label(
    root,
    text=f"2: {rank[1][0] if len(rank) > 1 else '-'} {rank[1][1] if len(rank) > 1 else '-'}",
)
rank3 = tk.Label(
    root,
    text=f"3: {rank[2][0] if len(rank) > 2 else '-'} {rank[2][1] if len(rank) > 2 else '-'}",
)
rank4 = tk.Label(
    root,
    text=f"4: {rank[3][0] if len(rank) > 3 else '-'} {rank[3][1] if len(rank) > 3 else '-'}",
)
rank5 = tk.Label(
    root,
    text=f"5: {rank[4][0] if len(rank) > 4 else '-'} {rank[4][1] if len(rank) > 4 else '-'}",
)
